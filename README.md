...
1. 페이지별 레이아웃 설정이 번거롭다.
   레이아웃을 적용해야하는 페이지마다 `Page.getLayout`를 설정해줘야하는 단점이 있다.
2. 데이터 페칭이 페이지 컴포넌트에 집중된다.
   `getServerSideProps`를 통해서 페이지 컴포넌트가 props 형태로 전달 받는데, 만약 자식 컴포넌트가 많아질 경우 전부 props로 전달해줘야한다.
3. 불필요한 컴포넌트들도 JS Bundle에도 포함된다.
   사전 렌더링 이후에 하이드레이션을 위해 JS Bundle이 전달되는데, 이때 하이드레이션이 필요없는 컴포넌트도 포함되어 있어서 불필요하게 Bundle 용량이 커지게 된다.
